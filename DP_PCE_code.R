...
pacotes = c("tidyverse", "mvtnorm", "MASS", "plyr",
            "mniw", "LaplacesDemon")
...
A <- rbinom(n, 1, pnorm(0.5+reg1+reg2-0.5*Xpred[,3]*Xpred[,4]))

M_1 <- rnorm(n, 3 - 0.5*reg1 + 0.5*reg2 + 1*abs(Xpred[,3]+1) + 1.5*Xpred[,4], 0.1)
M_0 <- rnorm(n, (0) - 0.5*reg1 + 0.5*reg2 + 1*abs(Xpred[,3]+1) + (0)*Xpred[,4], 0.1)
M_out <- ifelse(A==1, M_1, M_0)   

S <- M_1-M_0

Y_1 <- rnorm(n, 1*reg1 + 1.5*reg2 - 1 + 2*abs(Xpred[,3]+1)  - 1.5*(S+2)^2 + 2*Xpred[,4], 0.3)
Y_0 <- rnorm(n, 1*reg1 + 1.5*reg2 -(0) + 2*abs(Xpred[,3]+1) - 0.3*(S+2)^2 + 2*Xpred[,4], 0.3)
Y_out <- ifelse(A==1, Y_1, Y_0)   
...
for (i in 1:iter) {
  ...
  output[[i]] <- cbind(potential_Y, potential_M)
  colnames(output[[i]]) <- c('Y_0', 'Y_1', 'M_0', 'M_1')
  print(i)
}
...
# potential_M_0 plot
bbb <- rep(NULL, n)
bbb[which(A==0)] <- ctrl_M
bbb[which(A!=0)] <- potential_M_0[[i]]
plot(bbb, M_0)

# potential_M_1 plot
ccc <- rep(NULL, n)
ccc[which(A==1)] <- trt_M
ccc[which(A!=1)] <- potential_M_1[[i]]
plot(ccc, M_1)

# potential_y_0 plot
ddd <- rep(NULL, n)
ddd[which(A==0)] <- ctrl_y
ddd[which(A!=0)] <- potential_y_0
plot(ddd, Y_0)

# potential_y_1 plot
eee <- rep(NULL, n)
eee[which(A==1)] <- trt_y
eee[which(A!=1)] <- potential_y_1
plot(eee, Y_1)
